...
from anonlm.schema import PIIResponse
...
def _parse_pii_response_from_message(raw_message: Any) -> PIIResponse:
    ...
    try:
        return PIIResponse.model_validate_json(raw_text)
    except ValidationError:
        match = re.search(r"\{[\s\S]*\}", raw_text)
        if match:
            try:
                return PIIResponse.model_validate_json(match.group(0))
            except ValidationError:
                pass
...
def make_process_chunk_node(
    ...
    def process_chunk_node(state: PIIState) -> dict[str, Any]:
        if llm is None:
            raise RuntimeError(
                "Missing dependency or model client. Install project dependencies and configure API key."
            )
        ...
        response = raw_response if isinstance(raw_response, PIIResponse) else _parse_pii_response_from_message(raw_response)
...
def routing_fn(state: PIIState) -> Literal["process_chunk", "anonymize"]:
